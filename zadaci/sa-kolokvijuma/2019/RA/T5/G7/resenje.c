...
UKRAS *create_new_item(char naziv_prodavnice[], char naziv_ukrasa[],char vrsta_ukrasa[], double cena) {
...
void read_list_from(FILE *in, UKRAS **head) {
    ...
    while(fscanf(in, "%s %s %s %lf", naziv_prodavnice, naziv_ukrasa, vrsta_ukrasa, &cena) != EOF) {
        UKRAS *new = create_new_item(naziv_prodavnice, naziv_ukrasa, vrsta_ukrasa, cena);
