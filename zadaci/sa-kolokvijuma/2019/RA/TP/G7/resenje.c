...
BROWNIE *create_new_item(char naziv[], unsigned kolicina, double cena, char proizvodjac[]) {
...
void read_tree_from(FILE *in, BROWNIE **root, double *ukupna_cena) {
    ...
    while(fscanf(in, "%s %u %lf %s", naziv, &kolicina, &cena, proizvodjac) != EOF) {
...
void print_best_to(FILE *out, char *proizvodjac, char *best){
    fprintf(out, "Namirnica sa najvecom gramazom proizvodjaca %s je: %s\n",proizvodjac, best);
}
