...
void transform(
    struct kafa_st kafe[],
    struct kafa_pred_ispit_st kafe_pred_ispit[],
    int n,
    double kolicina_kofeina);
void snimi(
    FILE *out,
    struct kafa_st kafe[],
    struct kafa_pred_ispit_st kafe_pred_ispit[],
...
int main(int arg_num, char **args)
{
    ...
    {
        printf(
            "USAGE: %s IN_FILENAME OUT_FILENAME KOLICINA_KOFEINA\n",
            args[0]);
        exit(3);
    }
    ...
    double kolicina_kofeina)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        kafe_pred_ispit[i].kolicina_kofeina =
            broj_kafa * kafe[i].kolicina_kofeina;
    }
}
