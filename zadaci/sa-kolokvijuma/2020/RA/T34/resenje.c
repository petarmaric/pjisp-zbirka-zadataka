...
int main(int arg_num, char **args) {
    ...
    printf("%s %.2lf", skijalista[najbolji].naziv, skijalista[najbolji].cena_dnevne_karte * br_dana);
    fclose(in);
...
void ucitaj_skijalista(FILE *in, skijaliste skijalista[], int *n) {
    *n = 0;
    while(fscanf(
        in, "%s %lf",
         skijalista[*n].naziv,
        &skijalista[*n].cena_dnevne_karte
    ) != EOF) {
        (*n)++;
    }
}
...
int nadji_najbolju_ponudu(skijaliste skijalista[], int n){
    ...
    int i;
    for(i = 1; i < n; i++){
        if(skijalista[najbolji].cena_dnevne_karte > skijalista[i].cena_dnevne_karte){
            najbolji = i;
        }
    }
    return najbolji;
